...
import com.makyg.ge.Adapter.StoryAdapter
import com.makyg.ge.Model.Story
...
class StoryActivity : AppCompatActivity(), StoriesProgressView.StoriesListener {
    ...
    private fun addViewToStory(storyId: String){
        FirebaseDatabase.getInstance().reference.child("Story").child(userId!!).child(storyId).child("views").child(currentUserId).setValue(true)
    }
